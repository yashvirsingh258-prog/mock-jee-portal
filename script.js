...
// 4. THE CLEANER (FIXED FOR LINE BREAKS)
function cleanMath(str) {
    if (!str) return "";
    return str
        .replace(/\\\\\\\\/g, '\\') // Fixes quadruple slashes
        .replace(/\\\\/g, '\\')     // Fixes double slashes
        .replace(/\\n/g, '<br>')    // TARGETED FIX: Converts the string "\n" to an HTML break
        .replace(/\n/g, '<br>');    // Fallback for real newline characters
}
...
// 7. DETAILED SOLUTION (PRESERVED LOOK & FEEL)
...
function showFinalResultOnly() {
    ...
    // RESTORED NAVY BLUE STICKER / PREMIUM LOOK
    res.innerHTML = \`
