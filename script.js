...
// 3. AUTHENTICATION HANDLERS (STRICT GATEKEEPING)
window.handleLogin = async function() {
    const email = document.getElementById('login-email').value.trim();
    const pass = document.getElementById('login-pass').value.trim();

    // FAIL-SAFE 1: Internal logic stop
    if (!email || !pass) {
        alert("CRITICAL: Email and Password cannot be empty.");
        return; 
    }
    ...
    if (error) {
        ...
        // FAIL-SAFE 2: Only change view if user is verified by database
...
window.handleSignup = async function() {
    ...
    else alert("Success! Please confirm your email before logging in.");
};
...
window.startExam = async function() {
    if (!currentUserEmail) { alert("Please login first"); setView('login'); return; }
    
    const sub = document.getElementById('subject-select').value;
    activeBank = questionBank[sub] || questionBanks.mathematics;
...
window.finalSubmission = async function() {
    ...
    document.getElementById('score-val').innerHTML = `
        <div style="display: flex; justify-content: center; margin-bottom: 30px;">
            <div style="background: linear-gradient(135deg, #0b4a8f, #1e90ff); color: white; padding: 20px 40px; border-radius: 50px; text-align: center;">
                <div style="font-size: 2.5rem; font-weight: bold;">${score} / ${totalQuestions}</div>
